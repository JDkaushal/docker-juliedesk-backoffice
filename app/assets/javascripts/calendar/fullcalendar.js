...
function BasicView(element, calendar, viewName) {
	...
	function buildHeadHTML() {
		var headerClass = tm + "-widget-header noborder ";
		var html = '';
		var col;
		var date;
		...
		if (showWeekNumbers) {
			html +=
				"<th class='fc-week-number noborder " + headerClass + "'>" +
		...
		for (col=0; col<colCnt; col++) {
			...
			html +=
				"<th class='fc-day-header noborder fc-" + dayIDs[date.day()] + " " + headerClass + "'>" +
...
function AgendaEventRenderer() {
	...
	function renderSlotSegs(segs, modifiedEventId) {
		...
		for (i=0; i<segCnt; i++) {
			...
			if (isRTL) {
				...
				left = right - width;
				//seg.borderTop = "border-top-width: 4px;";
				//console.log("RTL");
			}
			else {
			...
			/*
			if(isOverLap) seg.borderTop = "border-top-width: 4px;";
			else seg.borderTop = "";
			*/
	...
	function slotSegHtml(event, seg) {
		...
		var mod = "";
		if(seg.overlap) mod = "border-top-width: 2px; border-left-width: 2px;";
		else mod = "border-top-width: 0px; border-left-width: 0px;";
		...
		var borderColor =
			...
			*/
			...
			var closeIcon = "";
			if(event.beingAdded && event.editable) closeIcon = "<img class=\"delete-event-button\" src=\"" + '' + "\"/>"
		...
		html +=
			...
			" style=" +
				...
				//"margin-top: 1px; border-top-width:0px;"+
				...
				"'" +
			">" +
			"<div class='fc-event-inner" + otherClasses + "' style='border-left-style:solid; border-left-width:3px; border-left-color:" + borderColor +"'>" +
			closeIcon +
...
function flattenSlotSegLevels(levels) {
	...
	for (i=0; i<levels.length; i++) {
		...
		for (j=0; j<level.length; j++) {
			if(i>0){
				//console.log("overlap");
				level[j].overlap = true;
			}
			else{
				level[j].overlap = false;	
			}
			segs.push(level[j]);
		}
	}
