class MessagesThread < ActiveRecord::Base
  ...
  TRAINING_EMAILS = %w(kaushal@juliedesk.com)
  ...
  def computed_data message_classifications=nil
    if @computed_data.blank?
      ...
      @computed_data = {
          ...
          online_meeting_provider: last_message_classification.try(:online_meeting_provider)
      }
    end
  ...
  def calendars_synced?
    ...
    if attendees_emails_to_check.blank?
      ...
      accounts = attendees_emails_to_check.map{|email| Account.accounts_cache_for_email(email)}.reject{|x| x.blank? || x['state'] == 'ghost'}
