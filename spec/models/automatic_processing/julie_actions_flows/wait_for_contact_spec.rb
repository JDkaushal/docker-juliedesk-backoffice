...
describe AutomaticProcessing::JulieActionsFlows::WaitForContact do
  ...
  subject(:flow) { AutomaticProcessing::JulieActionsFlows::WaitForContact.new(julie_action) }
  ...
  describe 'trigger' do
    ...
    it 'should trigger the correct actions' do
      expect(flow).to receive(:get_wait_for_contact_template).with(
          {:locale=>"en"}
      ).and_return('mocked_generated_template')
      flow.trigger

      expect(julie_action.text).to eq("mocked_generated_template")
    end
  end
end