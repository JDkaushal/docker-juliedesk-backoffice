    ...
    window.submitClassification = function () {
        ...
        if(window.featuresHelper.isFeatureActive('ai_dates_verification')) {
            ...
            if (filterCanVerifyV3 && hasSelectedDatesToVerify) {
                ...
                aiDatesVerificationManager.verifyDatesV6($.extend({}, verifyParams)).then(
                    function(response) {
                        console.log('response Verify Dates V5', response);
                    }, function(error) {
                        console.log('error Verify Dates V5', error);
                    }
                );

                aiDatesVerificationManager.verifyDatesV5($.extend({}, verifyParams)).then(
                    function(response) {
                        ...
                        if(!response.error && response.status != 'fail') {
                            ...
                            _.each(response.dates_validate, function (validated, date) {
                                if (validated && moment.tz(date, 'UTC').isAfter(now)) {
