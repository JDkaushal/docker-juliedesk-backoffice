...
describe AutomaticProcessing::AutomatedMessageClassification do
  ...
  let(:ai_main_raw_response) {
    {"language_detected"=>"en",
     "location_confidence"=>nil,
     "request_confidence"=>0.97,
     "request_proba"=>0.98,
     "request_model_date"=>"Mon Aug  7 13:28:19 2017",
     ...
     "constraints_data"=>[
         {"constraint_when_nature"=>"custom",
          ...
          "attendee_email"=>"owner@email.com"}
     ],
     "appointment_confidence"=>0.96,
     "client_on_trip"=>nil,
     "appointment_model_date"=>"Tue Aug  8 09:19:04 2017",
     "algo_duration"=>0,
     "duration"=>nil,
     "dates_to_check_confidence"=>0.98,
     "dates_to_check"=>["2018-03-23T16:00:00+01:00"],
     "appointment_proba"=>0.91
    }
  }

  let(:main_message_interpretation) { FactoryGirl.create(:main_classification, detected_classification: 'ask_availabilities', raw_response: ai_main_raw_response.to_json) }
  ...
  describe 'Instantiation' do
    ...
    subject(:classif) {
      AutomaticProcessing::AutomatedMessageClassification.new(
    ...
    it 'should hold a data_holder object' do
      expect(subject.data_holder).to eq(data_holder)
    end

    context 'No data holder provided' do
      let(:data_holder) { nil }

      it 'should raise an appropriate error' do
        expect{
          subject
        }.to raise_error(AutomaticProcessing::AutomatedMessageClassification::NoDataHolderError)
      end
    end
  end
  ...
  describe 'process_message_id' do
    ...
    it 'should create the correct MessageClassification' do
      ...
      expect(last_message_classif.message).to eq(processed_message)
      expect(new_classif.data_holder).to eq(data_holder)

      expect(last_message_classif.appointment_nature).to eq('appointment')
      expect(last_message_classif.summary).to eq("Mtg Owner Corp [Thread Owner] <> Grabou Corp [John Bernard] <> Babtou Corp [Babtou Fragile]")
      ...
      expect(last_message_classif.attendees).to eq("[{\"email\":\"threadOwner@owner.fr\",\"fullName\":\"Thread Owner\",\"firstName\":\"Thread\",\"lastName\":\"Owner\",\"gender\":\"M\",\"company\":\"Owner Corp\",\"isPresent\":true,\"isClient\":true,\"usageName\":\"Monsieur Thread Owner\"},{\"email\":\"john.bernard@grabou.fr\",\"fullName\":\"John Bernard\",\"firstName\":\"John\",\"lastName\":\"Bernard\",\"gender\":\"M\",\"company\":\"Grabou Corp\",\"isPresent\":true,\"isClient\":false,\"usageName\":\"Monsieur John Bernard\"},{\"email\":\"babtou@fragile.fr\",\"fullName\":\"Babtou Fragile\",\"firstName\":\"Babtou\",\"lastName\":\"Fragile\",\"gender\":\"M\",\"company\":\"Babtou Corp\",\"isPresent\":true,\"isClient\":false,\"usageName\":\"Monsieur Babtou Fragile\"}]")
      ...
      expect(last_message_classif.call_instructions).to eq("{\"target\":\"later\",\"support\":\"mobile\",\"targetInfos\":{}}")
