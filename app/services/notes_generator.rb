class NotesGenerator
  ...
  def initialize(message_classification)
    @message_classification = message_classification
  end

  def generate_notes
    [
        @message_classification.is_virtual_appointment? ? notes_category(get_translation(:call_instructions), generate_call_instructions) : notes_category(get_translation(:address_complement), generate_address_complement),
  ...
  def generate_call_instructions
    ...
    account = @message_classification.send(:account)
  ...
  def generate_address_complement
    @message_classification.send(:account_address).try(:[], 'address_complement')
  end
  ...
  def generate_contacts_infos
    @message_classification.get_present_attendees.reject(&:is_thread_owner).map do |attendee|
  ...
  def get_translation(key)
    ...
    }[key][@message_classification.locale.to_sym]
  end
