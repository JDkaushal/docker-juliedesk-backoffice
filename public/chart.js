    ...
    var Line = function(data,config,ctx){
        ...
        function drawScale(){
            ...
            for (var i=0; i<data.labels.length; i++){
                ...
                if(config.scaleShowGridLines && i>0 && (!config.scaleHorizontalNumberOfBars || i % config.scaleHorizontalNumberOfBars == 0)){
    ...
    var LineDoubleY = function(data,config,ctx){
        ...
        function drawLines(animPc){
            for (var n_scale = 1; n_scale<=2; n_scale++) {
                ...
                for (var i=0; i<actual_dataset.length; i++){
                    ...
                    ctx.lineWidth = actual_dataset[i].strokeWidth || config.datasetStrokeWidth;
                    ctx.beginPath();
        ...
        function drawScale(){
            ...
            for (var i=0; i<data.labels.length; i++){
                ...
                //Check i isnt 0, so we dont go over the Y axis twice.
                if(config.scaleShowGridLines && i>0 && i < (data.labels.length-1) && (!config.scaleHorizontalWidth || i % config.scaleHorizontalWidth == 0)){
                    ctx.lineWidth = config.scaleGridLineWidth;
                    ctx.strokeStyle = config.scaleGridLineColor;
                    ctx.lineTo(yAxisPosX + i * valueHop, 5);
                }
                ...
                for (var j=0; j<actual_calculatedScale.steps; j++){
                    ...
                    if (config.scaleShowLabels){
                        ...
                        if (n_scale == 1) {
                            ...
                            ctx.fillText(thousand_separator(actual_calculatedScale.labels[j]),yAxisPosX+xAxisLength+distance_Y2,xAxisPosY - ((j+1) * actual_scaleHop));
                        }
                    }
                }
            }
